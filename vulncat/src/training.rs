...
impl Trainer {
    ...
    pub fn train(&self, model: &mut AutoEncoder, dataset: &CommitDataset) -> Result<TrainingStats> {
        ...
        let mut optimizer = AdamW::new(varmap.all_vars(), adamw_config)?;
        ...
        for epoch in 0..self.config.epochs {
            let (train_loss, train_errors) =
                self.train_epoch(model, &train_data, &mut optimizer)?;
    ...
    fn train_batch(
        ...
        for commit in batch {
            ...
            let embeddings = self
                .preprocessor
                .create_embeddings(&[processed], &self.tokenizer)?;
        ...
        let input_tensor =
            Tensor::from_vec(batch_embeddings.clone(), &[batch_size, input_dim], device)?
                .to_dtype(DType::F32)?;
    ...
    fn update_baseline_and_max_error(&self, train_errors: &[f32], val_errors: &[f32]) {
        ...
        if !all_errors.is_empty() {
            ...
            println!(
                "Updated baseline error: {:.4}, max error: {:.4}",
                baseline, max
            );
        }
    }
